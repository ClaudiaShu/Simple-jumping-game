...
var Game = function () {
  ...
  this.config = {
    ...
    cubeColor: Colors.yellow,
    cubeWidth: 3, // 方块宽度
    cubeHeight: 1, // 方块高度
    cubeDeep: 3, // 方块深度
    energyColor: Colors.red, //能量颜色
    energyWidth: 0.5, // 能量宽度
    energyHeight: 0.5, // 能量高度
    energyDeep: 0.5, // 能量深度
    jumperColor: Colors.blue, //jumper颜色
...
Game.prototype = {
  init: function () {
    ...
    this._createEnergy() // 加一个能量
    this._createEnergy() // 再加一个能量
  ...
  restart: function () {
    ...
    // 删除所有方块
    var length_c = this.cubes.length
    for(var i=0; i < length_c; i++){
      this.scene.remove(this.cubes.pop())
    }
    // 删除所有能量
    var length_e = this.energies.length
    for(var i=0; i < length_e; i++){
      this.scene.remove(this.energies.pop())
    }
    // 删除jumper
  ...
  **/
  ...
  //碰撞检测
  _checkCollectE: function(){
    if (this.energies.length > 1) {
      ...
      // 下一个能量的位置
      var pointB = {
        x: this.energies[this.energies.length - 1].position.x,
        z: this.energies[this.energies.length - 1].position.z
      }
      var distanceL;  // jumper和下一个能量的坐标轴距离
      
      distanceL = (Math.pow(pointO.x - pointB.x, 2)+Math.pow(pointO.z - pointB.z, 2))
      ...
      if (distanceL<should) {
        ...
        this.energyStat.doExist = false;
        this.scene.remove(this.energies[this.energies.length - 1])
      }else {
        this.energyStat.doExist = true;
      }      
    }

    console.log("jumper is", pointO)
    console.log("energyB is", pointB)
    console.log("distance is", distanceL)
    console.log(this.energyStat.doExist)
...
//=================================================================================================//
    ...
    // MTLLoader.load('./src/model/blue.mtl', function(materials) {// 返回一个包含材质的对象MaterialCreator
    //   console.log(materials);//obj的模型会和MaterialCreator包含的材质对应起来
    //   OBJLoader.setMaterials(materials);
    //   OBJLoader.load('./src/model/blue.obj', function(obj) {
    //     console.log(obj);
    //     obj.scale.set(1, 1, 1); 
    //     this.jumper = obj;
    //     this.jumper.castShadow = true; // 产生阴影

    //     mesh.add(this.jumper)
    //   })
    // })
//=================================================================================================//
  ...
  _createCube: function () {
    ...
    cubePos_x = mesh.position.x;
    cubePos_y = mesh.position.y;
    cubePos_z = mesh.position.z;
  ...
  //新增一个能量，有存在，不存在两个状态
  _createEnergy: function(){
    
    var randX = this._createRandomX(); 
    var randZ = this._createRandomZ(); 
    ...
    var bodyGeometry = new THREE.CubeGeometry(this.config.energyDeep, this.config.energyWidth, this.config.energyHeight)
    //bodyGeometry.translate(0, 1, 0 )
    // var material = new THREE.MeshLambertMaterial({color: this.config.energyColor})
    var material = new THREE.MeshLambertMaterial({color: cubeMaterial})
    var mesh = new THREE.Mesh(bodyGeometry, material)
    mesh.castShadow = true; // 产生阴影
    mesh.receiveShadow = true;    // 接收阴影

    
    if( this.energies.length ) {
        mesh.position.x = cubePos_x+randX;
        mesh.position.y = cubePos_y+1.5;
        mesh.position.z = cubePos_z+randZ;
    }
    ...
    function render() {
      mesh.rotation.y += 0.01;
        ...
        if(ts == 100){
            var colorArray = new Float32Array([Math.random(),Math.random(),Math.random()]);
            mesh.material.color.fromArray(colorArray);
            ts = 0;
        }
        renderer.render(this.scene,this.camera);
    ...
    this.energies.push(mesh)

    if (this.energies.length > 6) {      
      this.scene.remove(this.energies.shift())
    }
    this.scene.add(mesh);
  },
  _render: function () {
  ...
  _createMaterial: function(){ // 生成材质/贴图
    ...
    function Chess() {
      var texture = new THREE.TextureLoader().load('src/pic/chess.png');
      texture.wrapS = texture.wrapT = THREE.RepeatWrapping; // 指定重复方向为两个方向
    ...
    function Bat() {
      var texture = new THREE.TextureLoader().load('src/pic/bat.png');
      texture.wrapS = texture.wrapT = THREE.RepeatWrapping; // 指定重复方向为两个方向      
      // texture.repeat.set(2, 2); // 设置重复次数都为4
      return {
        material : new THREE.MeshBasicMaterial( { map: texture } ),
  ...
  _createGeometry: function(){ // 生成几合体
    ...
    else if(randN < 0.8&&randN > 0.6){
      obj.geometry = new THREE.CylinderGeometry(1,2,1,8,2,false,0,Math.PI*2)
      obj.type = 'CubeGeometry';
    }
    else if(randN < 0.6&&randN > 0.3){
      obj.geometry = new THREE.CylinderGeometry(2,2,1,6,1,false,0,Math.PI*2)
      obj.type = 'CubeGeometry';
    }
    else if(randN < 0.3){
  ...
  _createRandomX: function(){ // 随机生成x偏移
    //var x = Math.random();
    var x = this._randomNum(-1,1);
    return x;
  },
  _createRandomZ: function(){ // 随机生成z偏移
    //var z = Math.random();
    var z = this._randomNum(-1,1);
    return z;
  }
}